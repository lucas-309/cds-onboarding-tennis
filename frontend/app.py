...
def introduction():
    ...
    st.text("Our dataset describes all the matches from the 2024 ATP Tour (including information like match winner, game points, etc.). We chose it as we were interested in predicting the winner of the 2025 Australian Open, and we thought the data from 2024 would be the most useful. Important features include the ranking points of the winner and loser at the time of the match (winner_rank_points and loser_rank_points, respectively), the ages of both players (winner_age, loser_age), and point information (such as points played by the winner and loser).")
...
def manipulation():
    ...
    st.text("We decided to clean the data by removing all rows with NaN values. We made this decision because we knew that those NaN values would do nothing but harm our prediction model, so we decided the best choice was to remove the match entirely.")

def visualization():
    st.title("Data Visualization")
    st.image("../plots/histogram_w_svpt.png")
    st.text("This is a histogram of the frequency of the total service points played by the winner in each match. As shown by the graph, most winners played between 50 and 75 service points in a match.")
    
    st.image("../plots/histogram_w_ace.png")
    st.text("This is a histogram of the frequency of the number of aces served by the winner in each match. As shown by the graph, most winners played under 10 aces. The graph is surprisingly bimodal.")
